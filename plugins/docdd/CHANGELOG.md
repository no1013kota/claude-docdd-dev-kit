...
- **dev-loop**: タスクの参照先の文書に、運営者が書き換えたままコミットしていない変更があれば、そのタスクのコミットに含める（更新日と変更履歴も直す）。docs を書き換えたら、コミットせずに `/docdd:tasks-from-docs` → `/docdd:dev-loop` と進めばよい。
- **security-audit**: 直すときに `/docdd:doc-sync` を回し、権限の決まりやエラーの応答が変わったら仕様書も同じコミットで直す。

### 変更

- **update-kit が `"notifyUpdates": false` を消していた**（不具合）: `.docdd/manifest.json` に足した「更新のお知らせを止める」設定が、`/docdd:update-kit` や init のたびに消えていた。消さないように直した。
...
- **不安にさせる言い回しを減らした**: 「確かめていない」「見込み」「壊れる」などをやめ、何をすればよいかを書く形にした。使える条件（何が要るか・何に対応していないか）は残した。
- **表の最初の列を、その行が何についての行か（ファイル・フォルダのパス、スキル、道具など）にした**。見出しの空欄も無くした。
- **見出し「控えと戻し方（壊れたときに戻せるようにする）」を「控えと戻し方」にした**（リンクも合わせた）。
...
- 手で直すもの（任意。update-kit は既存の節の中の文言を変えない）: `CLAUDE.md`・`docs/`・`tasks/BACKLOG.md` の文言や表の見出しを新しい雛形にそろえたいときは、Claude Code に「docdd の新しい雛形と見比べて、文言をそろえて」と頼む。「起票する」行を自分で書き換えていたら、`/docdd:tasks-from-prd` を `/docdd:tasks-from-docs` に直す。
...
- **詳しい説明書に「どこまで使えるか」**: 使える場所の表（Desktop の WSL・クラウドのセッション・Cowork を含む）、hook に要る Claude Code の版（PowerShell では 2.1.147 以上）、表の見出しを訳すと機能が黙って効かなくなること、プロジェクトの種類ごとに検証コマンドがどこまで自動で埋まるかの表、機能ごとに要るもの（release・本番 DB の控え・依存の脆弱性・DB の検証・操作の検証・refactor と speed-up）、気をつけること（モノレポ・Python の仮想環境・終わらないテストのコマンド・Windows の改行・確かめていないもの）。
...
- **Desktop アプリの入れ方**: 「配布元の追加はターミナルで」と書いていたが、Desktop（＋ → Plugins）と VS Code（`/plugins`）は画面からも入れられる。画面での入れ方は確かめていないので、ターミナルでの入れ方も残した。
...
- 制約の一部は、直せば無くなる（例: 依存の追加の確認が `pip3` に効かない、Windows の改行で update-kit の判定がずれる、Node.js と Python が同じフォルダにあると片方しか推定しない）。今回は説明に留め、直すかは別に決める。
