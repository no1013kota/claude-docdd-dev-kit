...
配布元は GitHub の `no1013kota/claude-docdd-dev-kit` です（Claude Code から見た配布元の名前＝マーケットプレイス名は `claude-docdd-dev-kit`、プラグイン名は `docdd`。手順2の `@` の後ろにこの名前を付けます）。GitHub のアカウントは要りません。
