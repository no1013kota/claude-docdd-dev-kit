...
- **docs を自分で書き換えたときは、何も書かずに `/docdd:add-task` と打ち、書き換えた所をタスクにします**（PRD・requirements・自分で置いた仕様書のどれでも）。docs を書き換えただけでは、アプリは変わりません。書き換えたら、コミットせずにそのまま打ちます（add-task はコミットしていない書き換えの節を読み、そのタスクを進める `/docdd:dev-loop` が、書き換えた文書もコミットに含めます。表記の直しなど、タスクにならない書き換えは add-task が聞いてからコミットします）。書き換えを先にコミットしたときは、直前のコミットなら `/docdd:add-task HEAD~1`（2 つ前からなら `HEAD~2`）と打ちます。
